...
import { NgbDatepickerI18n, NgbDateStruct, NgbCalendar, NgbDatepickerConfig,
     NgbModal, ModalDismissReasons } from '@ng-bootstrap/ng-bootstrap';
...
export class NouvelleReservationComponent implements OnInit {
    ...
    onDateRetourSelect(event)
    {
        ...
        let dd = new NgbDate(this.dateDepart.year,this.dateDepart.month,this.dateDepart.day).toString();
        let dr = new NgbDate(this.dateRetour.year,this.dateRetour.month,this.dateRetour.day).toString();
        let diff = Date.parse(dr) - Date.parse(dd);
...
// open modal confirmation annulation reservation
  ...
  editerReservation(response)
  {
    this.reset();
    this.btnSave = false;
            let dateDepart = response.reservationDto.dateDeDepart;
            let dateRetour = response.reservationDto.dateDeRetour;
            let dateDepartSplitted = dateDepart.split("/", 3);
            let dateRetourSplitted = dateRetour.split("/", 3);
            let dateDepartformatted = dateDepartSplitted[2]+'/'+dateDepartSplitted[1]+'/'+dateDepartSplitted[0];
            let dateRetourformatted = dateRetourSplitted[2]+'/'+dateRetourSplitted[1]+'/'+dateRetourSplitted[0];
            ...
            this.description = {modele:response.voitureDto.modele,marque:response.voitureDto.marque,
